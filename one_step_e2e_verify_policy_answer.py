#!/usr/bin/env python3
"""
서비스 모드 브리지 기동 → 정책 질문 → 정책 도구 호출 + 답변 반영 확인 (재현용)

키 준비 방식:
- 모델 키(UPSTAGE_API_KEY): hermes 개발 프로필 auth.json에서 access_token을 가져온다.
- 정책 키(ON_YOUTH_API_KEY, GOV24_API_KEY): 프로젝트 루트 .env를 policy_fetch.load_api_keys()로 읽는다
  (환경변수 우선, 없으면 같은 폴더 .env 보조).

실행 전 확인:
- 개발용 hermes 프로필(/Users/JWoon/.hermes/profiles/one-step/auth.json)에 유효한 Upstage 토큰이 있어야 한다.
- 프로젝트 루트 .env에 ON_YOUTH_API_KEY, GOV24_API_KEY가 있어야 한다.
- 이 스크립트는 키 값을 출력하지 않는다(길이만 출력).

주의:
- 이 스크립트는 키 주입을 위해 프로세스 환경변수에 민감 키를 일시 설정한다.
- 서비스 모드 브리지를 포트 9097로 띄우고, 종료 시 프로세스를 정리한다.
"""
...
# 스크립트 파일 위치를 프로젝트 루트로 잡는다.
# 이 스크립트는 프로젝트 루트에 있으므로 script_dir == project root가 된다.
PROJECT_ROOT = os.path.abspath(os.path.dirname(__file__))
BRIDGE = os.path.join(PROJECT_ROOT, "bridge_server.py")
PROFILE = os.path.join(PROJECT_ROOT, "deploy", "ServiceProfile")

# 모델 키: hermes 개발 프로필 auth.json에서 가져온다.
# 기본 경로는 개발자 로컬 기준이며, 다른 환경에서는 HERMES_AUTH_PATH로 바꿀 수 있다.
DEFAULT_AUTH_DIR = os.path.expanduser("~/.hermes/profiles/one-step")
AUTH_PATH = os.environ.get("HERMES_AUTH_PATH", os.path.join(DEFAULT_AUTH_DIR, "auth.json"))
